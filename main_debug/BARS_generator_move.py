...
Time = int(time()*1000)
ras = 50
#import os.path # 파일이 존재하는지 확인하기 위해 -> 경로확인
#import datetime
import webbrowser
...
control_memory_mode = 0 #  메모리 작업: 1,2 - 시뮬레이터, 3,4 - 데뷔, 5,6 - 작업
control_memory_mode_i = 1 # 난이도
control_memory_mode_over = -1 # 결과
control_move = go_color # 누가 먼저 가는지(먼저 움직이는 쪽 판단)
control_add = 1 # 추가할 색상\체커\퀸
control_edit = 0 #  내가 메뉴에 있는지 없는지
control_vs = 2 #  누가 로봇과 플레이를 하는지 (로봇은 누구를 위한 게임인가요? 1또는 2또는 3)###########################################################################
go_error = 0 # 컨트롤러에 오류 메시지를 표시하려면( 0 ~ 12) 
control_back = 0 #  예/아니오 "뒤로"라고 말하는 방법
control_forward = 0 # #예/아니오로 "전달"을 호출하는 방법 (아직 구현안된듯.)
control_revers_monitor = 0 # 1 - 필드의 일반 표시(화이트 체커가 아래, 검은 체커가 위). 2 - 필드가 뒤집혀 있습니다(검은색이 플레이어에 더 가깝습니다).화이트 체커가 위, 검은 체커가 아래
control_revers_monitor_always = 0; #  이전과 동일, 어느 순간에만 9a 자동으로 과거)
#  위젯용
control_protocol = []
...
def qwerty():
    ...
    global t1
    out_window = Tk()
    #out_window.title('BARS 4.3')#버전 변경 및 in print_file() # 데모 스페셜
    ...
    def hello():
        print("Hello!")
        print("이것은 기성품 다이어그램을 만들기 위한 특수 프로그램입니다.")
        print("그리고 그들에게 전해지는 이야기,  이 프로그램의 SS")
        print()
    ...
    def  reboot_controller():
        ...
        global control_add
        global control_edit
        global control_vs # 누가 로봇과 플레이를 하는지 (로봇은 누구를 위한 게임인가요?) (1또는 2또는 3)
        global go_error
        global control_back
        global control_protocol
        global control_actions
    ...
    def print_go_operation():
        ...
        if (0): # (0 - нет) 1 - 출력(0 - 아니요)
            i = 0
            while (i < len(go_operation)):
                print(i+1, " ", end='')
                if (0): # 비전문적인 출력 (1 - 예, 0 - 전문적인 출력)
                    if (1): # 철회하는 방법 (1 - 인간 좌표 사용, 0 - 배열이 실제로 어떻게 보이는지)
                        u = 0
                        while(u < len(go_operation[i])):
                            if (go_operation[i][u]>2000):
                                print("<", end='')
                            if (1): # 1 - 표시(0 - 아니요) 어떤 검사기
    ...
    def print_history():
        ...
        if (0): #  1 - 출력(0 - 아니요)
            ...
            if (0): # 비전문적인 출력 (1 - 예, 0 - 전문적인 출력)
                while (i < len(history)):
                    print(i, " ", end='')
                    if (1): # 표시 방법(1 - 인간 좌표 사용, 0 - 배열이 실제로 어떻게 보이는지)
                        u = 0
                        while(u < len(history[i])):
                            if (history[i][u]>2000):
                                print("<", end='')
                            if (1): #1 - 어떤 검사기를 표시(0 - 아니오)
    ...
    def people_move(y,x):
        ...
        global t1
        kostil = 0
        if (klik[0][0]==-1 and area_monitor[y][x]!=9 and area_monitor[y][x]!=0): # 체커를 가져 가라
            clicked()
            t1.raise_exception() 
            reboot_controller()
            if (area_monitor[y][x]%10==go_color):
                ...
                go_error = 3
        elif (klik[0][0]!=-1 and klik[1][0]==-1 and klik[0][0]==x and klik[0][1]==y): # 아무데도 가지 않았다면 필요하다면 let go
            klik[0][0] = -1
            klik[0][1] = -1
            moved_checkers = -1
            go_error = 0
        elif (klik[0][0]!=-1 and klik[1][0]==-1 and area_monitor[y][x]%10==go_color): # 아무데도 가지 않았다면 필요하다면 체커를 섞어
            klik[0][0] = x
            klik[0][1] = y
            #if ((control_vs==1 or go_color==2 and control_vs==3) and control_edit==0 and control_revers_monitor==1):
            #    x = 7 - x
            ...
            #    y = 7 - y
            go_error = 0
        elif ((klik[0][0] != -1) and (area_monitor[y][x] != 9) and (area_monitor[y][x] == 0)): # 우리가 여전히 간다면
            flag = 0
            i = 0
            l = 0
            g = 0
    ...
    def test_task_memory():
        ...
        if (i<0):
            i = 0
        if (i<control_memory_mode_i*10): # 필요에 따라 시연을 하다
    ...
    def test_task_memory2():
        ...
        if (i<0):
            i = 0
        if (i<control_memory_mode_i2*10): # 필요에 따라 시연을 하다
            mas = deepcopy(area_monitor)
            ii = 0
            while(ii<3):
                area_monitor = deepcopy(history)
                ...
                if (control_edit==0):
                    if (control_vs==1 and go_color==2 or control_vs==2 and go_color==1 or control_vs==3):
                        ...
                        if (control_vs==go_color or control_vs==4):
                            control_revers_monitor_always = 0
                            if (len(history)>1): # 위젯이 즉시 업데이트되도록 일종의 버킷
                        ...
                        go_error = 3
                        reboot_controller()
                elif (control_memory_mode==6): #작업북 모드에서 "재생"용
                    if (control_revers_monitor_result()):
                        x = 7 - x
                        y = 7 - y
                    people_move(y,x)
                    ...
                    test_task_memory3()
                    #print(history)
                elif (area_monitor[y][x]!=9 and control_memory_mode!=5): # 필드에 체커 추가를 제어하는 ​​것(필드 클릭)
                    ...
                    if (x>=ras*9.25 and x<=ras*11.75):
                        if (control_memory_mode!=5):
                            ...
                            if (y>=ras*1.625 and y<=ras*1.875):
                                control_add = 0
                        if (control_memory_mode==0): #메모리 개발 모드
                            # 다운로드 링크
                            if (x>=ras*10.875 and y>=ras*6.25 and x<=ras*11.25 and y<=ras*6.625):
                                ...
                                webbrowser.open('https://github.com/SeungJae928/BARS-international-mju', new=2) # 깃허브 링크
                           #난이도 설명
                            if (x>=ras*11.375 and y>=ras*6.25 and x<=ras*11.75 and y<=ras*6.625):
                                ...
                                out_window.update()
                                sleep(0.1)
                                webbrowser.open('https://github.com/SeungJae928/BARS-international-mju', new=2) # 난이도 설명 주소
                            ...
                            if (y>=ras*8.25 and y<=ras*8.75): 
                                if (control_memory_mode==1): # "다음 작업"
                                    craft_task_memory() #새로운 일자리를 창출하다
                                elif (control_memory_mode==2): 
                                    control_memory_mode = 1
                                    test_task_memory() # 과제를 제출하다
                                elif (control_memory_mode==3): #  다음 작업
                                    craft_task_memory2() # 새로운 일자리를 창출하다
                                elif (control_memory_mode==4): 
                                    control_memory_mode = 3
                                    test_task_memory2() # 과제를 제출하다
                                elif (control_memory_mode==5): # 다음 작업
                                    control_memory_mode = 6
                                    control_vs = 3
                                    craft_task_memory3() # 새로운 일자리를 창출하다
                                #elif (control_memory_mode==6): 
                                #    control_memory_mode = 5
                                #    test_task_memory2() # 다음 작업
                            if (y>=ras*7.5 and y<=ras*8.0): #  나가기
                                control_memory_mode = 0
                    
            if (control_edit==0 or control_memory_mode==6): # 위젯에서 정보 서랍 호출
    ...
    def craft_protocol(a,b):  # 생각 위젯 1
        global control_protocol
        global history
        #i = 0
        control_protocol.clear()
        while (a<=b and len(control_protocol)<=10): # 모든 소방관에 대해 i<200
    ...
    def click3(event):
        ...
        if (x>=ras*9.25 and x<=ras*11.75 and y>=ras*0.5 and y<=ras*3.5): # 스크롤 위젯 1
            #print("1234567890")
            if (len(history)>1):
                control_protocol.clear()
                ...
                control_protocol.clear()
                control_protocol_flag = 0
        if (x>=ras*9.25 and x<=ras*11.75 and y>=ras*4 and y<=ras*6): # 스크롤 위젯 2
            if (len(go_operation)>7):
                ...
                #control_actions.clear()
                control_actions_flag = 0
        if (x>=ras*9.25 and x<=ras*11.75 and y>=ras*6.75 and y<=ras*7.25 and control_memory_mode==1): # 난이도를 스크롤하려면
            control_memory_mode_i = control_memory_mode_i - d
            if (control_memory_mode_i>32):
                control_memory_mode_i = 32
            if (control_memory_mode_i<1):
                control_memory_mode_i = 1
        if (x>=ras*9.25 and x<=ras*11.75 and y>=ras*6.75 and y<=ras*7.25 and control_memory_mode==3): #난이도를 스크롤하려면
            control_memory_mode_i = control_memory_mode_i - d
            ...
            if (control_memory_mode_i<1):
                control_memory_mode_i = 1
            area_monitor = deepcopy(debuts[control_memory_mode_i][1])
        if (x>=ras*9.25 and x<=ras*11.75 and y>=ras*6.75 and y<=ras*7.25 and control_memory_mode==5): # 난이도를 스크롤하려면
            control_memory_mode_i = control_memory_mode_i - d
    ...
    reboot_controller()
    #hello()
    if (control_vs==go_color or control_vs==4): # 글쎄, 너는 절대 모른다
        mozg()
        reboot_checkers()
        reboot_controller()
        go_operation = deepcopy(generator_move(area_monitor, go_color))
    ...
    canvas.bind("<Button-1>", click1)   # 버튼 1
    canvas.bind("<MouseWheel>", click3) # 마우스 휠을 돌리다
    canvas.bind("<Button-3>", click2)   # Л버튼 2
